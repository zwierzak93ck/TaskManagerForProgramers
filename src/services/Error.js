...
export const setEmailError = (emailRegExp, email) => {
    ...
    setError((!testRegularExpression(emailRegExp, email) && !email.includes('@')), 'Wrong Email. Missing "@" sign in "' + email + '".') ||
    setError((!testRegularExpression(emailRegExp, email) && email.includes('@')), 'Wrong Email. Missing part after "@" sign in "' + email + '".')
}
